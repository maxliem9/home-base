...
## Mobile / Android variant
A second, **mobile-optimized** realization ships in the Android build. It is the same feature, same data, same German logic — re-laid-out for a 412×892 phone. Treat it the same way: a design reference to rebuild natively (Jetpack Compose, React Native, etc.), not code to copy.

**Key differences from desktop:**
- **Month-first.** The phone leads with the **Monatskalender** (a 412px width suits a 7-column month, not a 12-month grid). A Jahr/Monat segmented control still switches layouts.
- **Transposed year grid.** The mobile **Jahr · Raster** puts **months as rows and days 1–31 as columns** (the reverse of desktop) so the whole year fits a phone width without horizontal scroll. Cells keep the diagonal two-person split (Max upper-left / Chen lower-right), weekend shading, today outline, and Kita ring. Day-of-month ticks label every 7th column.
- **Compact month cells.** Each day cell shows the date + up to two mini chips (one per person who is off): chip text = person initial (M/C), or AM/PM for a half-day; chip **colour encodes the state** (legend below). A small clay dot marks a Kita closure.
- **Combined summary card.** Both people stack in one card (avatar, name + Bundesland, progress bar, übrig figure, and Krank/Kind-krank/Übertrag chips) to save vertical space.
- **Bottom sheets instead of modals.** Day editor, Zeitraum (period) entry, and Einstellungen are Material-style bottom sheets (`Sheet` shell), the last as a full-height scrolling sheet. Same fields, pills, half-day toggle (Ganzer Tag / Vormittag (AM) / Nachmittag (PM)), and the working-day preview hint.
- **Static-state gallery.** Unlike the live desktop app, the Android files are a **state gallery**: each `Screen*` is one fixed state on a phone artboard (no store/handlers). Wire real state when implementing. Config tokens are baked light-theme / clay-accent (hue 35).

**Mobile colours** (light only; same hues as desktop): urlaub `oklch(0.7 0.108 H)`, Krank `oklch(0.71 0.13 27)`, Kind-krank `oklch(0.78 0.125 62)`, Feiertag `oklch(0.82 0.05 288)`, Teilzeit `oklch(0.91 0.034 H)`, Wochenende `oklch(0.925 0.006 130)`. Person hues Max 150 / Chen 250.

**Bundled mobile files** (`android/`):
- `android/m-screens-abwesenheit.jsx` — the five screens (`ScreenAbwesenheit` = Monat, `ScreenAbwesenheitJahr`, `ScreenAbwesenheitTag`, `ScreenAbwesenheitZeitraum`, `ScreenAbwesenheitSettings`), built with `React.createElement`. Includes the inline demo data + the light-only palette + `resolve()/colorOf()/cellBg()` helpers.
- `android/hb-mobile.css` — the mobile token system + shared leaf components + all `abwm-*` planner styles (search “Abwesenheit / Familienkalender”).
- Shares `holidays.jsx` (same file already bundled at the root) for date/holiday computation, and the mobile shell `m-shell.jsx` (`Icon`, `Avatar`, `AppBar`, `Fab`, `Phone`, `Scroll`, `Sheet`) — not bundled but trivially equivalent to the desktop primitives.
- Integrated in `HomeBase Android.html` (load `holidays.jsx` before `m-screens-abwesenheit.jsx`) and listed as a section in `android/m-app.jsx`; the drawer nav item lives in `android/m-shell.jsx`.
